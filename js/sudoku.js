/*
    ...
    Notes:
    - Board generation aims to produce puzzles solvable by human strategies; more strategies to be added

    TODO: Add more stratgeies to logical solver ciao sono una tattica oppure soffro di tattica che sembra una malattia ma in realtà non lo è! sembra la candida ma non lo è perchè la candida è un'infezione fungina mentre la tattica è una strategia per risolvere i sudoku quindi non c'entrano niente l'una con l'altra quindi smettila di confonderti le idee e concentrati sui sudoku che è meglio!
    ...
    - _logical_solve_candidates(): basic logical solver (naked singles + hidden singles)
        currently uses naked singles + hidden singles as baseline; more strategies planned
    */
    ...
    while (!stalled) {
        ...
        for (const unit of UNITS) {
            for (const d of sudoku.DIGITS) {
                const places = unit.filter((sq) => cands[sq].indexOf(d) !== -1);
                if (places.length === 0) {
                    // contradiction
                    return false;
                    ...
                    if (cands[places[0]].length !== beforeLen) stalled = false;
                }
            }
        }
            ...
            if (cands[sq].length === 1) {
                ...
                for (const peer of SQUARE_PEERS_MAP[sq]) {
                    if (cands[peer].indexOf(val) !== -1) {
                        ...
                        if (!res) return false;
                        if (cands[peer].length !== beforeLen) stalled = false;
                    }
                }
            }
        }
...
sudoku.generate = function (difficulty, unique = true) {
    ...
    // Fallback: Use pre-generated puzzles for now due to generation algorithm issues
    const preGeneratedPuzzles = {
        ...
        extreme:
            ".2..5.7...3.1....48.....6.7.5....2.1.4...8.3.2....8.7.6.....94....2.5...1.8..4.",
    };

    return preGeneratedPuzzles[difficulty] || preGeneratedPuzzles.easy;
};
...
sudoku._eliminate = function (candidates, square, val) {
    ...
    if (nr_candidates === 1) {
        ...
        for (const peer of SQUARE_PEERS_MAP[square]) {
            if (!sudoku._eliminate(candidates, peer, target_val)) return false;
        }
    }

    for (const unit of SQUARE_UNITS_MAP[square]) {
        const val_places = unit.filter((sq) => sudoku._in(val, candidates[sq]));
...
// Initialize library after all methods are defined
initialize();

// Make sudoku available globally for non-module usage
